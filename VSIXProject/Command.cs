    ...
    internal sealed class Command
    {
        ...
        private void StartNotepad()
        {
            try
            {
                ...
                var projectPath = Path.Combine(projectDir ?? throw new InvalidOperationException(), "bin\\Release"); ;
                var command = $" dotnet nuget push {projectPath}\\*.nupkg -k Benchint -s http://10.3.1.240:8080/nuget";
                ...
                p.StandardInput.WriteLine($"dotnet pack -s {Path.Combine(projectDir, Path.GetFileName(project.FullName))} -c Release");  //先转到系统盘下
                ...
                p.Close();
                p.Dispose();
                ShowMessageBox("nuget-push success");
            }
            catch (Exception ex)
            {
